...
const AI = (() => {
  ...
  async function call(system, user, { json = true, maxTokens = 1600 } = {}) {
...
Return {"suggestions":[...]}. Only events you are confident genuinely exist. If unsure of a detail, say so in "risk".`);
  }
...
{"verdict": one of "go","go if clustered","skip",
 ...
 "blindSpot": one thing the score structurally cannot see about this event that a human should check}`);
  }
...
"tell":"the single detail that settles it"}`);
  }
...
{"arc": one sentence on what has actually changed across these meetings — cite the specific detail that moved,
 ...
 "avoid": one thing NOT to do with this person, based on the history}`);
  }
  ...
  async function parseCapture(raw, confName) {
    return call(
...
cold = polite, took a leaflet, no real signal.`);
  }

  return { cfg, hasKey, call, mineSignals, discover, interpretScore, adjudicateMatch, relationshipArc, parseCapture };
})();
